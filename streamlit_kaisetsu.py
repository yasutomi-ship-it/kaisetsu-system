...
def validate_expl(expl):
    ...
    # 自己矛盾の検出：「正」の解説が正解を否定する文言を含んでいないか
    NEG_PHRASES = ('とはいえな', '該当しない', 'ではない。', 'には当たらない',
                   'とは言えな', 'ものではない')
    for it in items:
        if it.get('正誤') == '正':
            body = str(it.get('内容', ''))
            if any(ph in body for ph in NEG_PHRASES):
                warns.append(
                    f"選択肢{it.get('番号')}を『正』としていますが、解説文が正解を否定する内容です"
                    "（AIの判断が矛盾している可能性）。解答・解説をご確認ください。")
                break
    return expl, warns
...
【自己矛盾の禁止（出力前に必ず自己チェック）】
- "正" とした選択肢の解説に「最も低いとはいえない」「該当しない」など、正解を否定する文言を
  書いてはならない。"正"の解説は必ず「なぜそれが答えなのか」を述べる。
- 出力前に確認：①"正"の解説が答えである理由になっているか ②"誤"の解説が"正"と矛盾しないか
  ③"解答"＝"正"の番号か。矛盾があれば判定をやり直すこと。

【構造の同定を誤りやすい例（特に注意）】
- N を含んでいても「アミド」とは限らない。カルボニル炭素に直接 N が結合（C(=O)−N）して
  初めてアミドである。C(=O)−O−C(=N)−N のように、カルボニルの隣が O のものはエステル型であり
  アミドではない（例：O−アシルイソ尿素＝DCC縮合の活性中間体で、反応性はむしろ非常に高い）。
- 一般論の序列に引きずられて「アミドがあるはず」と決めつけない。選択肢にアミドが無い問題も多い。

【重要】
- PDFの図・構造式を実際に読み取り、選択肢と構造の対応・正誤を正確に判定すること。
  （求核アシル置換の反応性序列＝酸ハロゲン化物＞酸無水物・活性エステル≧O−アシルイソ尿素
   ＞通常のアルキルエステル＞アミド。
   ・O−アシルイソ尿素（DCC活性中間体）：活性化アシル種で反応性は高い＝低い側ではない。
   ・フェニルエステル等の活性エステル：フェノラートが良い脱離基で、アルキルエステルより高い。
   ・通常のアルキルエステル（プロパン酸エチル等）：アルコキシドは脱離能が低く、
     選択肢にアミドが無ければ、これが「最も反応性が低い」になることが多い。）
